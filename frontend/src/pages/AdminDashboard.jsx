...
export default function AdminDashboard() {
  ...
  async function fetchUsers() {
    try { const res = await listUsers(); setUsers(res.data.data); } catch {}
  }
  async function fetchReports() {
    try { const res = await getReports(); setReports(res.data.data); } catch {}
  }
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-6">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <form onSubmit={handleCreateUser} className="grid grid-cols-2 gap-3">
                ...
                <input placeholder="Password (min 6)" type="password" value={newUser.password} onChange={(e) => setNewUser({ ...newUser, password: e.target.value })} required className="border rounded px-3 py-2 text-sm" />
                ...
                <input placeholder="Class ID (optional)" value={newUser.classId} onChange={(e) => setNewUser({ ...newUser, classId: e.target.value })} className="border rounded px-3 py-2 text-sm col-span-2" />
        ...
        {tab === 'reports' && (
          <div className="bg-white rounded-lg shadow p-6">
            ...
            {!reports.length ? <p className="text-sm text-gray-400">No data yet.</p> : (
              <table className="w-full text-sm">
                <thead><tr className="text-left text-gray-500 border-b"><th className="pb-2">Student</th><th className="pb-2">Class</th><th className="pb-2">Present</th><th className="pb-2">Late</th><th className="pb-2">Absent</th><th className="pb-2">Total</th></tr></thead>
                <tbody>
                  {reports.map((r, i) => (
