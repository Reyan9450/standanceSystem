...
import {
  startSession, getActiveSession, getSessionAttendance,
  approveAttendance, rejectAttendance, manualMark, submitAttendance, listUsers
} from '../services/api';
...
export default function TeacherDashboard() {
  ...
  // Load teacher's classes from users list (admin assigns classId to teacher)
  useEffect(() => {
    // We'll let teacher type classId manually for MVP
  ...
  async function checkActiveSession() {
    if (!classId) return;
    try {
      const res = await getActiveSession(classId);
  ...
  async function handleApprove(id) {
    try {
      ...
      setMsg(err.response?.data?.error?.message || 'Failed to approve');
    }
  }
  ...
  async function handleReject(id) {
    try {
      ...
      setMsg(err.response?.data?.error?.message || 'Failed to reject');
    }
  }

  async function handleManual(studentId, status) {
    try {
      await manualMark(studentId, session._id, status);
      fetchAttendance(session._id);
    } catch (err) {
      setMsg(err.response?.data?.error?.message || 'Failed to mark');
    }
  }

  async function handleSubmit() {
    if (!window.confirm('Finalize attendance? This will mark all remaining students absent and close the session.')) return;
    try {
      ...
      setMsg(err.response?.data?.error?.message || 'Failed to submit');
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-blue-700 text-white px-6 py-3 flex justify-between items-center">
        <span className="font-semibold">Teacher Dashboard — {user.name}</span>
        ...
        {session && (
          <div className="space-y-4">
            ...
            <AttendanceSection
              title="Verified"
              color="green"
              records={attendance.verified}
              renderActions={() => null}
            />

            {/* Unverified */}
            <AttendanceSection
              title="Unverified (Pending Review)"
              color="yellow"
              records={attendance.unverified}
              renderActions={(r) => (
                <div className="flex gap-2">
                  <button onClick={() => handleApprove(r._id)} className="text-xs bg-green-500 text-white px-2 py-1 rounded hover:bg-green-600">Approve</button>
                  <button onClick={() => handleReject(r._id)} className="text-xs bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600">Reject</button>
                </div>
              )}
            />
            ...
            <div className="bg-white rounded-lg shadow p-4">
              ...
              {attendance.absent?.length === 0 ? (
                <p className="text-sm text-gray-400">All students have marked attendance.</p>
              ) : (
                    ...
                    {attendance.absent.map((s) => (
                      <tr key={s._id} className="border-b last:border-0">
                        ...
                        <td className="py-2">
                          <select
                            defaultValue=""
                            onChange={(e) => e.target.value && handleManual(s._id, e.target.value)}
...
function AttendanceSection({ title, color, records, renderActions }) {
  const colors = {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4">
      <h3 className={`font-medium mb-3 ${colors[color]}`}>{title} ({records.length})</h3>
      {records.length === 0 ? (
