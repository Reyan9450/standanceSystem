    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto p-6 space-y-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          {!activeSession ? (
            ...
            <div className="space-y-2">
              <p className="text-sm text-gray-600">Active session: <span className="font-medium">{activeSession.subject}</span></p>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          {!history.length ? <p className="text-sm text-gray-400">No records yet.</p> : (
            <table className="w-full text-sm">
              <thead><tr className="text-left text-gray-500 border-b"><th className="pb-2">Date</th><th className="pb-2">Class</th><th className="pb-2">Subject</th><th className="pb-2">Status</th></tr></thead>
              <tbody>
                {history.map((r) => (
                  <tr key={r._id} className="border-b last:border-0">
                    ...
                    <td className="py-2">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium capitalize ${statusColors[r.status] || ''}`}>{r.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
